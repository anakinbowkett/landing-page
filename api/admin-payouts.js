...
<div class="header"><div class="logo"><h1>Monturalearn</h1><p>monturalearn@gmail.com</p></div><div class="badge">PAYMENT RECEIPT</div></div>
<div class="grid">
<div class="section"><h3>RECEIPT DETAILS</h3><p><strong>Receipt Number:</strong> ${receiptNumber}<br><strong>Invoice Number:</strong> ${ambassador.invoice_number}<br><strong>Date Paid:</strong> ${paymentDate.toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'})}<br><strong>Payment Method:</strong> PayPal</p></div>
<div class="section"><h3>PAY TO</h3><p><strong>${ambassador.first_name} ${ambassador.last_name}</strong><br>${ambassador.address_line1 || ''}<br>${ambassador.address_line2 ? ambassador.address_line2 + '<br>' : ''}${ambassador.city || ''}, ${ambassador.postcode || ''}<br>United Kingdom<br>${ambassador.email}</p></div>
</div>
<div class="banner"><div class="label">Amount Paid on ${paymentDate.toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'})}</div><div class="amount">£${amount}</div></div>
<table><thead><tr><th>Description</th><th style="text-align:right">To Pay</th></tr></thead>
<tbody><tr><td><strong>Monturalearn Startup Circle UGC Creator</strong><br><span style="color:#6b7280;font-size:14px">Joined: ${new Date(ambassador.joined_at).toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'})}</span></td><td style="text-align:right;font-weight:600">£${amount}</td></tr></tbody></table>
<div class="totals"><div class="row"><span>Subtotal:</span><span>£${amount}</span></div><div class="row final"><span>Amount Paid:</span><span>£${amount}</span></div></div>
<div class="footer"><p>This receipt confirms payment has been made via PayPal.</p><p>For any queries, contact: monturalearn@gmail.com</p></div>
</body></html>`;
